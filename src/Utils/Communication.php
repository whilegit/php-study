...
class Communication{
	 ...
	 * @return array             错误返回 return array('success'=>false, 'msg' =>'xxx错误原因xxx');  正确返回array(查看ihttp_response_parse函数)
	 */
	public static function ihttp_request($url, $post = '', $extra = array(), $timeout = 60, $proxy = null) {
	 ...
	 */
	public static function ihttp_email($config, $to, $subject, $body) {
		static $mailer;
		/*
		 * 
		 */
	...
	protected static function ihttp_response_parse($data, $chunked = false) {
		...
		if($chunked && $ischunk) {
			$rlt['content'] = self::ihttp_response_parse_unchunk($split1[1]);  //chunked重新组装
		} else {
